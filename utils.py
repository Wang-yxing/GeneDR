...
def load_data_from_database(dataset='DATASET2', testing=False, rating_map=None, post_rating_map=None, ratio=1.0,use_features=False,with_addds=False,posneg_ratio=1.0,per_path=None,walk_len=None,cross_fold=0.9):
    """
    ...
    # drug_disease_file = os.path.join('data/',dataset,'processed/','data_array_dr_di.csv')
    if not os.path.exists(f'data/{dataset}/processed/links.npz'):
        if dataset == 'DATASET1':
            drug_disease_file = os.path.join('data/',dataset,'raw/','drug_disease_association.csv')
            ...
            lagcn_drug = pd.read_csv(
                'data/DATASET1/raw/drug.csv', usecols=['drug_id','drugbank_id']
            )
            ...
            # data_dr_di.to_csv('data/DATASET1/processed/data_dr_di.csv',index=False)
            # data_array_dr_di = np.loadtxt(drug_disease_file, delimiter=',', skiprows=1, dtype='str')         
            data_array_dr_di = np.array(data_dr_di,dtype=str)
        ...
        if with_addds:
            ...
            os.path.join('data/',dataset,'raw/','8k.csv'),dtype=str
            )
            ...
            dis_gene = pd.read_csv(
                os.path.join('data/',dataset,'raw/','dis_gene.csv'),header=None,usecols=[0,2],dtype=str
            )
            dis_gene.insert(loc=1,column='relation',value='ML')
            dis_gene.columns=['sourceid','relation','targetid']
            che_gene = pd.read_csv(
                os.path.join('data/',dataset,'raw/','drug_gene.csv'),header=None,usecols=[1,2,4],dtype=str
            )
            che_gene.columns=['sourceid','targetid','relation']
            che_gene = CTDintegrate(che_gene)[['sourceid','relation','targetid']]

            che_gene.insert(loc=1,column='sourcetype',value='chemical')
            che_gene.insert(loc=4,column='targettype',value='gene')

            dis_gene.insert(loc=1,column='sourcetype',value='gene')
            dis_gene.insert(loc=4,column='targettype',value='disease')

            data_array_all = pd.concat([data_DGD[['sourceid','sourcetype','relation','targetid','targettype']],che_gene,dis_gene])
            
            
            # data_array_all.to_csv('data/DATASET1/processed/data_array_all.csv',index=False)
            # data_array_all = pd.read_csv(
            #         os.path.join('data/',dataset,'processed/','data_array_all.csv'),dtype=str
            #     )

            temp = data_array_all[data_array_all['sourcetype']=='gene']['sourceid'].drop_duplicates()
            temp = temp[temp.notnull()]
            temp2 = data_array_all[data_array_all['targettype']=='gene']['targetid'].drop_duplicates()
            ...
            # data_array_all = np.array(data_array_all,dtype=str)
            u_nodes_ratings_p =  np.array(data_array_all['sourceid'],dtype=str)
            v_nodes_ratings_p =  np.array(data_array_all['targetid'],dtype=str)
            ratings_p = np.array(data_array_all['relation'],dtype=str)
            ...
            # infile.write('%d\t%d\n'%(num_gene+num_drug+num_disease,351851))
            infile.write('%d\t%d\n'%(num_gene+num_drug+num_disease,351851))
            rating_mx_gg = np.full((num_gene, num_gene), neutral_rating, dtype=np.int32)
            for i in data_array_all[(data_array_all['sourcetype']=='gene')&(data_array_all['targettype']=='gene')][['sourceid','relation','targetid']].values:
            ...
            cg = data_array_all[\
                (data_array_all['sourcetype']=='chemical')&(data_array_all['targettype']=='gene')|\
                    (data_array_all['sourcetype']=='gene')&(data_array_all['targettype']=='chemical')
            ...
            dg = data_array_all[\
                (data_array_all['sourcetype']=='disease')&(data_array_all['targettype']=='gene')|\
                    (data_array_all['sourcetype']=='gene')&(data_array_all['targettype']=='disease')
                    ][['sourceid','relation','targetid']]
            for i in dg.values:
                d = i[0]
                g = i[2]
            ...
            if use_features == True:
                if dataset == 'DATASET1':
                    ...
                    drug_sim = np.loadtxt('data/DATASET1/raw/initial_embedding/drug_sim.csv',delimiter=',',dtype=np.float32)
                    dis_sim = np.loadtxt('data/DATASET1/raw/initial_embedding/dis_sim.csv',delimiter=',',dtype=np.float32)
                    ...
                    lagcn_disease = pd.read_csv(
                        'data/DATASET1/raw/disease.csv', usecols=['disease_id']
                    )
                ...
                embeddingfile = f'data/{dataset}/raw/initial_embedding/'
                embedding = json.load(open(f'{embeddingfile}/embed.vec'))
                embedding_node = np.array(embedding['ent_embeddings.weight'])
                embedding_rel = np.array(embedding['rel_embeddings.weight'])
                name2id = {i.strip().split()[1]:i.strip().split()[0] for i in open(f'{embeddingfile}/entity2id.txt')}
                relname2id = {i.strip().split()[1]:i.strip().split()[0] for i in open(f'{embeddingfile}/relation2id.txt')}
            ...
            json.dump(gene_dict,open(f'data/{dataset}/processed/gene_dict.json','w'))
            json.dump(drug_dict,open(f'data/{dataset}/processed/drug_dict.json','w'))
            json.dump(disease_dict,open(f'data/{dataset}/processed/disease_dict.json','w'))
            json.dump(rating_dict_p,open(f'data/{dataset}/processed/relation_dict.json','w'))
            ...
            json.dump(dic,open(f'data/{dataset}/processed/allnode_dict.json','w'))
            ...
            np.savez(f'data/{dataset}/processed/features',u_features=u_features, v_features=v_features, rel_features=rel_features, gene_features=gene_features)
            np.savez(f'data/{dataset}/processed/matrix',rating_mx=rating_mx,allow_pickle=True)
            np.savez(f'data/{dataset}/processed/links',train=[u_train_idx,v_train_idx],test=[u_test_idx, v_test_idx],val=[u_val_idx, v_val_idx])
            np.savez(f'data/{dataset}/processed/labels',train=train_labels,test=test_labels,val=val_labels)
            np.savez(f'data/{dataset}/processed/paths',paths=paths,allow_pickle=True)
            np.savez(f'data/{dataset}/processed/sim',drug_sim=drug_sim,dis_sim=dis_sim)
            np.savez(f'data/{dataset}/processed/class_values',class_values=class_values)
    else:
        feature = np.load(f'data/{dataset}/processed/features.npz')
        u_features, v_features, rel_features, gene_features = feature['u_features'],feature['v_features'],feature['rel_features'],feature['gene_features']
        rating_mx = np.load(f'data/{dataset}/processed/matrix.npz',allow_pickle=True)['rating_mx']
        links = np.load(f'data/{dataset}/processed/links.npz')
        [u_train_idx,v_train_idx],[u_test_idx, v_test_idx],[u_val_idx, v_val_idx] = links['train'], links['test'], links['val']
        labels = np.load(f'data/{dataset}/processed/labels.npz')
        train_labels,test_labels,val_labels = labels['train'], labels['test'], labels['val']
        paths = np.load(f'data/{dataset}/processed/paths.npz',allow_pickle=True)['paths']
        sim = np.load(f'data/{dataset}/processed/sim.npz')
        drug_sim,dis_sim = sim['drug_sim'],sim['dis_sim']
        drug_dict = json.load(open(f'data/{dataset}/processed/drug_dict.json'))
        disease_dict = json.load(open(f'data/{dataset}/processed/disease_dict.json'))
        node_features = np.concatenate((gene_features,u_features,v_features))
        class_values = np.load(f'data/{dataset}/processed/class_values.npz')['class_values']
